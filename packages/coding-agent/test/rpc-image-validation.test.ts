 ...
 * `follow_up` RPC commands use upstream conversion and optional resizing.
 * Processing failures become indexed notes; supported MIME types are trusted.
 */
...
describe("processRpcImages", () => {
	it("preserves a supported client-supplied mime type for an image that needs no resizing", async () => {
	...
	it("drops undecodable bytes during resizing and appends a note", async () => {
	...
	it("drops a PNG with a corrupt body during resizing", async () => {
	...
	it("uses the note as the whole message when the message is empty", async () => {
		...
		expect(result.message).toBe("[Image 1 omitted: could not be resized below the inline image size limit.]");
	});
	...
	it("keeps good images and notes bad ones by index", async () => {
		...
		expect(result.images![0].data).toBe(TINY_PNG);
		expect(result.message).toBe(
			"look at these\n\n[Image 2 omitted: could not be resized below the inline image size limit.]",
		);
	});
	...
	it("passes through bytes with a supported mime type without decoding when auto-resize is off", async () => {
		const garbage = Buffer.from("definitely not an image").toString("base64");
		const result = await processRpcImages("look at this", [image(garbage)], { autoResize: false });

		expect(result.images).toEqual([image(garbage)]);
		expect(result.message).toBe("look at this");
	});
});
