...
class RpcSocketClient {
	...
	constructor(socket: Socket, maxBacklogBytes: number, onClose: () => void) {
		this.socket = socket;
	...
	enqueue(record: RpcSocketRecord | RpcResponse): void {
		...
		// Evict only a client whose already-queued backlog is over the limit,
		// i.e. one that has stopped draining the socket while we keep
		// broadcasting to it. The check is before the record is added, so any
		// single record is always admitted regardless of its size: a large but
		// legitimate response (e.g. get_tree on a long session) must never be
		// the thing that closes the socket. queuedBytes only tracks records
		// still in our queue; once a record is handed to socket.write() it is
		// Node's buffer's problem, and a single oversized write is bounded by
		// V8's max string length at serialization, not here.
...
export async function runRpcSocketServer(
	...
	const server = createServer((socket) => {
		...
		client.enqueue(sessionChangedEvent());
		clients.add(client);
	...
	runtimeHost.addRebindSessionListener(async () => {
		...
		if (session.sessionId !== previousSessionId) {
			broadcastEvent(sessionChangedEvent());
		}
	});
