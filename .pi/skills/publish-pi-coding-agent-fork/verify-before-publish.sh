...
# Build steps can refresh tracked generated source files and AI model catalog
# files from mutable remote APIs. The coding-agent fork publish should stay
# minimal, so restore files that were clean when this script started before
# checking the committed release sources and packing.
restore_generated_model_catalogs
