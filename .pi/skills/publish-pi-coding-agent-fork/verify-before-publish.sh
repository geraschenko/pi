...
# The released workspace snapshot is trusted; dependency installs retain the age gate.
ai_package_tarball=$(npm pack "$ai_package_name@$ai_package_version" --min-release-age=0 --pack-destination "$model_catalog_snapshot_dir" --loglevel=error)
