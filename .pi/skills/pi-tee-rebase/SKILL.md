---
name: pi-tee-rebase
description: Rebase Anton's fork branch onto the latest upstream release tag, resolving known fork commits including rpc-socket mode, navigate_tree, RPC image inputs, RPC state fold, and npm fork packaging.
---
...
1. `Implementation of --rpc-socket (pi-tee)`
2. `Add navigate_tree rpc command`
3. `Process images sent through rpc mode the same as interactive mode`
4. `Implement RpcSessionState fold function.`
5. `Fork pi-coding-agent`
