...
Per-client compatibility objective: a socket client should be able to reuse a `--mode rpc` client implementation with minimal changes. Aside from the initial socket `hello` record and documented socket-only events, each client receives only:

- responses to commands it sent on that connection
- broadcast session events that would also be plausible in `--mode rpc` if another actor, such as a human user or extension, changed session state

The socket must not broadcast other clients' raw commands or command responses by default, because that would require clients to distinguish tee socket mode from stdio RPC mode.
...
- A newly connected client receives future events only, plus a connection-scoped `session_changed` record identifying the current session (see "New `--rpc-socket`-specific session replacement event"). All other catch-up is performed explicitly via commands such as `get_state`, `get_messages`, and `get_session_stats`.
...
- Socket mode emits a socket-only `session_changed` event on connect and on session replacement (see "New `--rpc-socket`-specific session replacement event").
...
### New `--rpc-socket`-specific session replacement event
...
- Sent once per connection immediately after the `hello` record, identifying the current session. A client can therefore track the active session with a single code path — remember the latest `session_changed` — without calling `get_state`.
- Broadcast to all connected clients whenever the active session is replaced, i.e. whenever the new session's `sessionId` differs from the previous one. This covers `/new`, `/resume`, `/fork`, `/clone`, and `/import` in the TUI and `new_session`, `switch_session`, `fork`, and `clone` over the socket.
- Not emitted for in-session operations that keep the same `sessionId`: prompting, compaction, and `/tree` navigation (`navigateTree` moves the leaf within the same session file).
- On replacement, the event is broadcast before any events from the new session reach clients.
- The announced `sessionFile` may not exist on disk yet: session files are written lazily on the first assistant message. Clients persisting the path for later revival must tolerate the file being absent until then.
...
### Output backpressure and the backlog bound

Each connected client has an unbounded-by-default risk: the server broadcasts
every event to every client, so a client that stops reading would otherwise
make the server buffer records for it forever. To bound that, each client has a
per-connection output backlog limit (`maxBacklogBytes`); a client whose backlog
exceeds it is disconnected (`socket.destroy`).

There are three distinct buffers in the outbound path, and the bound only
governs the first:

1. **The server's own queue** (`RpcSocketClient.queue`, measured by
   `queuedBytes`). Records that have been enqueued but not yet handed to
   `socket.write()`. This is the only thing `maxBacklogBytes` limits.
2. **Node's per-socket write buffer.** `socket.write()` always accepts and
   buffers the data; its return value is only an advisory backpressure signal
   (`false` once Node's buffer is over its `highWaterMark`). Once a record is
   written, its bytes leave our queue and become Node's buffer's responsibility
   — `queuedBytes` no longer counts them.
3. **The kernel socket buffers.** Node drains its buffer into the kernel as
   fast as the kernel accepts, which in turn depends on the client actually
   reading. When Node's buffer empties back to zero after a `write()` returned
   `false`, the `'drain'` event fires and the server resumes flushing.

The server can only tell a client is not draining indirectly: `write()` starts
returning `false`, `'drain'` stops firing, the flush loop parks, and newly
broadcast records pile up in the queue — that accumulation is what trips the
bound.

**A single record never closes the socket, regardless of its size.** The
backlog check is performed *before* a record is added to the queue, so the
record currently being enqueued is always admitted even if it alone exceeds
`maxBacklogBytes`. This is deliberate: a large but legitimate response (for
example `get_tree` or `get_entries` on a long session) must never be the thing
that severs the connection. Only a client that has *already* accumulated more
than `maxBacklogBytes` of undelivered backlog — i.e. one that has genuinely
stopped draining — is evicted. The true upper bound on a single record is V8's
maximum string length, enforced at JSON serialization time, not by this limit.

A consequence worth noting for debugging: when the bound does evict a client,
`socket.destroy()` discards Node's write buffer and tears down the connection,
so the server cannot deliver an error record explaining the eviction — the
offending client is, by definition, not reading anyway. The eviction reason is
therefore surfaced server-side (the `Error` passed to `destroy`), not to the
client, which simply observes a closed socket.
