...
The conversion + resize + attach + note-generation logic is factored into the shared `processImage(bytes, mimeType, { autoResizeImages })` function in `src/utils/image-process.ts`, used by all three `ImageContent`-producing paths — `processFileArguments`, `processRpcImages`, and the `read` tool — so the rules and their user-facing message text live in exactly one place. It returns either a processed image with `hints` (conversion and resize notes) or an omission `message`.

RPC rewrites the default `Image` label in those notes to the indexed `Image N` label so notes read `[Image 2 omitted: …]`. Each caller maps the result to its own surface: file args wrap notes in `<file>` tags, `read` prefixes `Read image file [mimeType]` and appends notes, and RPC appends notes to the message with the image index. Steps per image:

1. **Normalize the image to a model-supported inline format.** RPC passes the client-supplied `mimeType` into `processImage`; unsupported formats are converted to PNG when possible, otherwise RPC drops the image and appends the omission note. Invalid base64 needs no separate check: `Buffer.from(data, "base64")` yields garbage bytes that fail conversion/resize. The `read` tool still classifies image-vs-text through its **pluggable, path-based** `ReadOperations.detectImageMimeType` (a published extension point for remote/SSH backends), so it passes the already-known type into the shared pipeline.
...
- **PR summary**: RPC `prompt`/`steer`/`follow_up` images now go through the same conversion + resize pipeline as CLI file arguments and the `read` tool. Invalid or unshrinkable images are dropped and replaced by a visible note in the user message instead of poisoning the session; resizing honors the user's auto-resize setting.
