# Handoff: harden pi's RPC response serialization (crash class found in clauctl)
...
1. **Wire shape**: the session tree was serialized as *nested* JSON — one
...
## The fix pattern (apply the equivalent in pi)
...
2. **Never put recursion-depth-proportional nesting on the wire.** clauctl
   ...
   pointer); clients rebuild structure locally. If pi serializes any
   session-tree/AST-like structure whose depth grows with session length,
   the same overflow is latent.

## Why this matters for pi

pi's daemon/server presumably has the same shape: a socket loop that
...
- `/spec` if the pi change grows beyond a mechanical respond() guard (e.g.
